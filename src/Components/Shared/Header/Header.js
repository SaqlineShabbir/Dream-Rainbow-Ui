      ...
      <Navbar className="navbar " expand="lg">
        <Container>
          <Navbar.Brand className=" fw-bold" href="#home">
            {' '}
            <img src={logo} alt="" />
            Dream Rainbow <span className="my-color "></span>
          </Navbar.Brand>
          <Navbar.Toggle
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className=" ms-auto mt-3">
              <Nav.Link as={Link} className=" text-danger mx-4 " to="/home">
                Home
              </Nav.Link>
              <Nav.Link as={Link} className="" to="/aboutRoute">
                About
              </Nav.Link>
              <Nav.Link as={Link} className="mx-4" to="/services">
                Services
              </Nav.Link>
              {/* { !user.email &&   <Nav.Link as={Link} className="text-white fw-bold mx-3 nav-hover" to="/login">Log In</Nav.Link>} */}

              <Nav.Link as={Link} className="  dashboard-link" to="/dashboard">
                Dash Board
              </Nav.Link>
              ...
              {user.email ? (
                <Link to="/signup">
                  {' '}
                  <button onClick={logoutWithAlart} className="signup-button ">
                    Log Out
                  </button>
                </Link>
              ) : (
